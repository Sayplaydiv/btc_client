...
func New(bip44Path string, isSegWit bool, seed []byte, chainID int) (c *BTC, err error) {
	...
	c.DerivationPath, err = bip44.GetCoinDerivationPath(c.Symbol)
	if err != nil {
...
func (c *BTC) Sign(rawTx string, privateKeyWif []string) (signedRawTx string, err error) {
	...
	if msg.Inputs != nil {
		var wif []*btcutil.WIF
		for _, priv := range privateKeyWif {
			wif_, err := btcutil.DecodeWIF(priv)
			if err != nil {
				return "", err
			}
			wif = append(wif, wif_)
		...
		for i := 0; i < len(*msg.Inputs); i++ {
			if (*msg.Inputs)[i].ScriptPubKey == "" && (*msg.Inputs)[i].RedeemScript == "" { //未提供 ScriptPubKey, RedeemScript 则默认认为是单签,自动填充scriptPubKey
				if c.useSegWit {
					(*msg.Inputs)[i].RedeemScript, (*msg.Inputs)[i].ScriptPubKey = GenerateScriptPubKey4P2SHP2WPKH(wif[i].PrivKey.PubKey())
				} else {
					(*msg.Inputs)[i].ScriptPubKey = GenerateScriptPubKey4PayToPubkeyHash(wif[i].SerializePubKey())
				}
			}
		}
	}

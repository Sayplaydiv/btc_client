...
func TestSignTxOutputWitness(t *testing.T) {
	...
	fmt.Println("// P2SH Pay to witness pubkey hash (compressed)")
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			msg := fmt.Sprintf("%d:%d", hashType, i)
	...
	fmt.Println("// P2SH P2WSH Pay to pubkey (compressed)")
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			msg := fmt.Sprintf("%d:%d", hashType, i)
	...
	fmt.Println("// P2SH P2WSH Pay to pubkeyhash (compressed)")
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			msg := fmt.Sprintf("%d:%d", hashType, i)
